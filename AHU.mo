...
model AHU
  ...
  Buildings.Obsolete.Fluid.Sources.FixedBoundary bou(nPorts=1, redeclare
      package Medium = Air)
    annotation (Placement(transformation(extent={{98,-36},{78,-16}})));
  Buildings.Obsolete.Fluid.Sources.FixedBoundary bou1(nPorts=1, redeclare
      package Medium = Air)
...
equation
  ...
  connect(boundary1.m_flow_in, gain.y)
    annotation (Line(points={{-40,-18},{-55,-18}}, color={0,0,127}));
  ...
  connect(gain1.y, boundary.m_flow_in)
    annotation (Line(points={{55,40},{46,40}}, color={0,0,127}));
